...
/* Combo netcfg: UNCONFIGURED (waiting for Wi-Fi). Default for discovery ADV. */
static int sg_netcfg_status = TUYA_BLE_NETCFG_STATUS_UNCONFIG;

int tuya_ble_calc_adv_hash(const char *uuid, const char *auth_key, uint8_t *out_hash)
{
    uint8_t digest[16] = {0};
    uint8_t buf[MAX_LENGTH_UUID + MAX_LENGTH_AUTHKEY + MAX_LENGTH_PRODUCT_ID];
    const char *key_part = auth_key;
    size_t uuid_len;
    size_t key_len;
    size_t n = 0;

    if (uuid == NULL || out_hash == NULL) {
        return OPRT_INVALID_PARM;
    }

    /* Prefer AuthKey; if absent, fall back to stored PID. */
    if (key_part == NULL || key_part[0] == '\0') {
        if (sg_ble_service_params != NULL && sg_ble_service_params->pid[0] != '\0') {
            key_part = (const char *)sg_ble_service_params->pid;
        } else {
            key_part = "";
        }
    }

    uuid_len = strnlen(uuid, MAX_LENGTH_UUID);
    key_len = strnlen(key_part, MAX_LENGTH_AUTHKEY);
    memset(buf, 0, sizeof(buf));
    memcpy(buf + n, uuid, uuid_len);
    n += uuid_len;
    memcpy(buf + n, key_part, key_len);
    n += key_len;
    uni_md5_digest_tolal(buf, (unsigned int)n, digest);
    memcpy(out_hash, digest, 4);
    return OPRT_OK;
}

/**
 * Connect-Kit combo netcfg ADV (Security V4/V5).
 * Service Data 0xFD50 payload (8 bytes, no raw PID):
 *   [0]    Frame Control = 0x43 (UNCONFIG netcfg=0) or 0x00 (unbound)
 *   [1..2] Capability Flags = 0x00 0x0C (Wi-Fi 2.4/5G combo)
 *   [3]    Adv Flags = 0x10
 *   [4..7] Adv Hash = MD5(UUID||AuthKey)[0..3]
 * Scan RSP: MID 0x07D0 + ENC + ability + flag + AES-CBC(MD5) uuid_16.
 */
static int ble_adv_data_set(tuya_ble_service_params_s *dev_info,
                            TKL_BLE_DATA_T *p_adv, TKL_BLE_DATA_T *p_scan_rsp)
{
    OPERATE_RET rt = OPRT_OK;
    uint8_t segment_length = 0;
    const uint16_t ability = 0x000C;
    const uint8_t enc_mode = 0x00;
    const uint8_t adv_flag = 0x10;
    uint8_t adv_hash[4] = {0};
    uint8_t fc0;
    uint8_t svc_payload_len;

    if (dev_info == NULL || p_adv == NULL || p_scan_rsp == NULL ||
        p_adv->p_data == NULL || p_scan_rsp->p_data == NULL) {
        return OPRT_INVALID_PARM;
    }

    TUYA_CALL_ERR_RETURN(tuya_ble_calc_adv_hash((const char *)dev_info->uuid,
                                                 (const char *)dev_info->auth_key,
                                                 adv_hash));

    /* UNCONFIG (netcfg=0): FC=0x43; payload 43 00 0C 10 + AdvHash[4]. */
    fc0 = (sg_netcfg_status == TUYA_BLE_NETCFG_STATUS_UNCONFIG) ? 0x43 : 0x00;
    ...
    /* Incomplete list of 16-bit Service UUIDs: 0xFD50 */
    segment_length = 1;
    ...
    /* Service Data 0xFD50: FC | Cap | Flag | AdvHash[4]  → 8-byte payload */
    segment_length = 1;
    p_adv->p_data[p_adv->length + segment_length] = 0x16; segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0x50; segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0xFD; segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = fc0; segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = (uint8_t)(ability >> 8); segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = (uint8_t)(ability & 0xFF); segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = adv_flag; segment_length++;
    memcpy(&p_adv->p_data[p_adv->length + segment_length], adv_hash, 4);
    segment_length = (uint8_t)(segment_length + 4);
    p_adv->p_data[p_adv->length] = segment_length - 1;
    svc_payload_len = (uint8_t)(segment_length - 4); /* exclude len-slot + type + UUID */
    p_adv->length += segment_length;

    /* Scan RSP manufacturer 0x07D0 + AES-CBC(MD5) of uuid_16 */
    segment_length = 1;
    ...
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = enc_mode; segment_length++;
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = (uint8_t)(ability >> 8); segment_length++;
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = (uint8_t)(ability & 0xFF); segment_length++;
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = adv_flag; segment_length++;

    {
        uint8_t md5_in[20] = {0};
        uint8_t md5_data[16] = {0};
        uint8_t cipher[16] = {0};
        /* Hash the 8-byte Service Data payload (pad to 20 for V4 key derivation). */
        memcpy(md5_in, &p_adv->p_data[11], 8);
        uni_md5_digest_tolal(md5_in, 20, md5_data);
        tal_aes128_cbc_encode_raw(dev_info->uuid_16, 16, md5_data, md5_data, cipher);
        memcpy(&p_scan_rsp->p_data[p_scan_rsp->length + segment_length], cipher, 16);
        segment_length = (uint8_t)(segment_length + 16);
    }
    ...
    PR_DEBUG("tuya ADV FC=%02X cap=%04X flag=%02X AdvHash=%02X%02X%02X%02X svc_payload=%uB (no raw PID)",
             fc0, ability, adv_flag,
             adv_hash[0], adv_hash[1], adv_hash[2], adv_hash[3],
             (unsigned)svc_payload_len);
    return rt;
}

int tuya_iot_bt_initializer(void)
{
    if (sg_ble_service_params == NULL) {
        return OPRT_COM_ERROR;
    }
    sg_netcfg_status = TUYA_BLE_NETCFG_STATUS_UNCONFIG;
    PR_DEBUG("tuya_iot_bt_initializer: combo provisioning UNCONFIG");
    return OPRT_OK;
}

int tuya_ble_gatt_send_data_init(void)
{
    OPERATE_RET rt = tkl_ble_stack_init(TKL_BLE_ROLE_SERVER);
    if (rt != OPRT_OK) {
        PR_ERR("tuya_ble_gatt_send_data_init: GATT stack not ready");
        return OPRT_COM_ERROR;
    }
    PR_DEBUG("tuya_ble_gatt_send_data_init: GATT services registered (tuya_ble_gap_event_handler via port)");
    return OPRT_OK;
}

int tuya_ble_adv_property_set(tuya_ble_adv_property_e prop, int value)
{
    if (prop == TUYA_BLE_ADV_NETCFG_STATUS) {
        sg_netcfg_status = value;
        PR_DEBUG("tuya_ble_adv_property_set NETCFG_STATUS=%d", value);
        return OPRT_OK;
    }
    return OPRT_INVALID_PARM;
}
...
/**
 * Rebuild Connect-Kit ADV/ScanRSP (Adv Hash in 0xFD50, no raw PID).
 */
int tuya_ble_adv_data_reconfig(void)
{
    ...
    TKL_BLE_DATA_T adv_data = {.length = 0, .p_data = adv_data_buf};
    TKL_BLE_DATA_T scan_rsp_data = {.length = 0, .p_data = scan_rsp_buf};
    ...
    PR_DEBUG("tuya_ble_adv_data_reconfig done (combo AdvHash, netcfg=%d)", sg_netcfg_status);
    return rt;
}
...
int tuya_ble_adv_start(void)
{
    ...
    TUYA_CALL_ERR_RETURN(tuya_ble_adv_data_reconfig());
    ...
    PR_DEBUG("tuya_ble_adv_start (combo UNCONFIGURED)");
    return rt;
}

static int ble_service_adv_start(void)
{
    return tuya_ble_adv_start();
}
...
int tuya_ble_service_start(tuya_ble_service_init_params_t *init_params, ble_token_get_callback cb)
{
    ...
    // copy device information — keep trailing NUL in +1 buffers
    memset(sg_ble_service_params->pid, 0, sizeof(sg_ble_service_params->pid));
    memset(sg_ble_service_params->uuid, 0, sizeof(sg_ble_service_params->uuid));
    memset(sg_ble_service_params->auth_key, 0, sizeof(sg_ble_service_params->auth_key));
    memcpy(sg_ble_service_params->pid, init_params->pid,
           strnlen((char *)init_params->pid, MAX_LENGTH_PRODUCT_ID));
    memcpy(sg_ble_service_params->uuid, init_params->uuid,
           strnlen((char *)init_params->uuid, MAX_LENGTH_UUID));
    memcpy(sg_ble_service_params->auth_key, init_params->auth_key,
           strnlen((char *)init_params->auth_key, MAX_LENGTH_AUTHKEY));
    if (20 == strlen((char *)sg_ble_service_params->uuid)) {
    ...
    PR_DEBUG("license pid_len=%u uuid_len=%u auth_len=%u uuid16=%.16s",
             (unsigned)strlen((char *)sg_ble_service_params->pid),
             (unsigned)strlen((char *)sg_ble_service_params->uuid),
             (unsigned)strlen((char *)sg_ble_service_params->auth_key),
             (char *)sg_ble_service_params->uuid_16);
    ...
    TUYA_CALL_ERR_RETURN(tuya_ble_gatt_send_data_init());
...
int ble_service_loop(void)
{
    ...
    switch (first_node->cmd) {
        ...
        case (BLE_SVC_STATUS_CONNECT) :
            ...
            /* Idle disconnect timer (requires MultiTimerInstall in service_start). */
            if (MultiTimerInit(&sg_ble_service_params->timer_hdl, BLE_DISCONNECT_TIME_MS,
                               ble_disconnect, NULL) == 0) {
                MultiTimerStart(&sg_ble_service_params->timer_hdl, BLE_DISCONNECT_TIME_MS);
            } else {
                PR_ERR("MultiTimerInit failed on connect");
            }
        break;
